#!/usr/bin/env python3
"""
Fix common skill validation issues:
1. Replace invalid categories (agent, analysis, automation, ci, mojo, refactoring, uncategorized)
   with valid ones from: architecture, ci-cd, debugging, documentation, evaluation, optimization, testing, tooling, training
2. Fix Failed Attempts tables with missing columns
...
def fix_invalid_category(content: str, skill_name: str) -> tuple[str, bool]:
    """Replace invalid categories with valid ones."""
    changed = False

    # Extract category from frontmatter
    match = re.search(r"^category:\s*(\w+)", content, re.MULTILINE)
    if match:
...
def fix_failed_attempts_table(content: str) -> tuple[str, bool]:
    ...
    if not match:
        # Section doesn't exist - this is a different error, skip
        return content, changed
    ...
    if "|" in section_content:
        ...
        if len(lines) >= 2:
            ...
            # Missing columns - rebuild table with minimal entry
...
"""
        ...
        # Not a table - convert prose to minimal table
...
def main():
    ...
    for skill_file in skill_files:
        try:
            ...
            content, changed = fix_invalid_category(content, skill_name)

...
description: "Split E2E run directories into in_progress/ and completed/ phases with atomic promotion, agent git commit, and off-peak scheduling. Use when: (1) agent crashes or interrupted runs pollute results with phantom 0.0 scores, (2) judging must be isolated from in-flight runs, (3) rm -rf in_progress/ must be safe for clean restart."
...
| **Outcome** | ✅ Operational — all 5439 tests pass, 78.44% coverage, PRs #1738 and #1739 merged |
| **Verification** | verified-ci |
| **Project** | ProjectScylla |
| **PRs** | [#1738](https://github.com/HomericIntelligence/ProjectScylla/pull/1738), [#1739](https://github.com/HomericIntelligence/ProjectScylla/pull/1739) |

...
description: 'Move bandit skip flags from pre-commit hook CLI args to a .bandit INI
  config file so suppressions are visible when running bandit directly. Use when:
  bandit --skip flags live in .pre-commit-config.yaml making them invisible to developers,
  or pyproject.toml [tool.bandit] is desired but bandit doesn''t natively read it.'
category: ci-cd
date: 2026-03-07
version: 1.0.0

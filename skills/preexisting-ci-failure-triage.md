...
description: 'Triage CI failures to determine if they are pre-existing on main branch
  vs caused by PR changes. Use when: CI fails on a PR with only doc/config changes,
  determining whether to block merge, or reviewing Mojo runtime crashes unrelated
  to changed files.'
category: ci-cd
date: 2026-03-05
version: 1.0.0

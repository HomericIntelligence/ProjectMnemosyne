...
description: 'Fix systemic CI failures blocking all PRs, enable auto-merge on all
  open PRs, rebase all branches onto main, and merge conflicting skill content. Use
  when: main CI workflow fails with protected branch error, validate check has path
  filters preventing it from running, 100+ PRs need rebasing, or many PRs conflict
  on the same files.'
category: ci-cd
date: 2026-03-14
version: 1.0.0

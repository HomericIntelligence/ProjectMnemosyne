...
description: 'Efficiently triage and close security reviews for PRs containing only
  documentation or metadata changes. Use when: a security review is triggered on a
  PR that adds only markdown files, JSON metadata, or other static documentation with
  no executable code.'

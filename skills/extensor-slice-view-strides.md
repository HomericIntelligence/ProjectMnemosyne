...
description: 'Add stride-aware slice() view semantics and _nd_index_to_flat_offset
  to Mojo ExTensor for zero-copy slicing of non-contiguous tensors. Use when: implementing
  slice() that must work on transposed/strided views, fixing element access bugs in
  non-contiguous tensors, or adding a reusable view_with_strides primitive.'

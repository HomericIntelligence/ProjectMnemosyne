...
- **A rebase produces ONLY add/add conflicts and ALL conflicted files already exist on main with the canonical content** — resolving every conflict by taking main's version leaves zero staged diff vs `REBASE_HEAD`; the entire commit is fully superseded. Use `git rebase --skip` to drop the commit, NOT `--continue` (which would produce a superfluous empty commit). See § L.
...
- **`--skip` vs `--continue` decision**: after resolving all conflicts in a commit, run `git diff --cached REBASE_HEAD`. If the output is empty, the resolved tree is identical to what `REBASE_HEAD` had — the commit is fully superseded and `git rebase --skip` is correct. `git rebase --continue` would produce an empty commit that does nothing except clutter history. If the diff is non-empty, the commit still adds something new — use `--continue` (with `GIT_EDITOR=true`).
...
#### L. Fully-superseded commit: `--skip` not `--continue` (add/add all files already on main)

**Trigger**: A rebase produces `CONFLICT (add/add)` on multiple new files. After resolving each conflict by taking main's canonical version, there is nothing left for the commit to contribute — every staged file is identical to what is already on main. Running `--continue` would create a ghost empty commit; instead, **skip the commit entirely**.

**When this happens**: A sibling PR merged ALL of the same new files with the same content before this branch's rebase. The branch's commit is 100% superseded. Example: PR #1358 and PR #1365 (ProjectHephaestus) both implemented the CIDriver SRP decomposition. #1358 merged first, creating 5 new collaborator source files and 5 test files. When #1365 rebased, all 10 files conflicted as add/add — and main's versions were canonical.

**Step-by-step resolution**:

```bash
# 1. For each conflicted file, write main's version (ours = HEAD = main in rebase context)
git show REBASE_HEAD:<path/to/file.py> > /tmp/file.py
cat /tmp/file.py > <path/to/file.py>
git add <path/to/file.py>
# Repeat for every conflicted file

# 2. BEFORE deciding --skip vs --continue, verify the staged diff is zero:
git diff --cached REBASE_HEAD
# If empty (no output) → the commit is fully superseded → use --skip
# If non-empty → the commit still contributes something → use GIT_EDITOR=true git rebase --continue

# 3. Skip the superseded commit:
git rebase --skip

# 4. After rebase completes, confirm branch tip == main:
git diff origin/main HEAD
# Must be empty — zero diff confirms the branch tip is now identical to main
```

**Why `--skip` not `--continue`**: After taking main's version for every add/add conflict, the staged index produces zero delta vs the tree that already exists. `git rebase --continue` would detect this and create an empty commit (it would be the only commit containing no actual changes), cluttering history and potentially confusing squash-merge tooling. `git rebase --skip` drops the commit cleanly and advances the rebase pointer to the next commit (or finishes the rebase if this was the last commit).

**Outcome**: The branch tip becomes identical to `origin/main`. `git diff origin/main HEAD` shows zero output. The PR is effectively a no-op and should be closed as superseded — surface this to the human rather than silently pushing a zero-diff branch.

```bash
# After confirming zero diff, close the PR as superseded:
gh pr close <N> --comment "Superseded: identical change merged via #<sibling-PR-number>. Branch tip is now identical to main after rebase --skip."
```

**Do NOT confuse with K2 (parallel cluster-extraction)**: In K2, both branches have genuine unique work (the source files differ only in comments, but the test files have genuinely different test classes that must be merged). In the `--skip` scenario, the files are byte-for-byte identical — there is nothing to preserve from the branch.

**Checklist before `git rebase --skip`**:

- `git diff --cached REBASE_HEAD` → empty (required)
- `git show REBASE_HEAD -- <all conflicted files>` confirms you are looking at files with identical content on both sides (spot-check a few)
- `git log origin/main..HEAD --oneline` after the skip shows an empty or minimal log (0 commits remaining → branch tip == main)
...
| `git rebase --continue` after resolving add/add conflicts with main's version | After taking main's files for all conflicts in a fully-superseded commit, ran `GIT_EDITOR=true git rebase --continue` | When ALL staged changes after conflict resolution are zero vs `REBASE_HEAD`, `--continue` would produce a superfluous empty commit cluttering history (or git may warn "nothing to commit") | Check `git diff --cached REBASE_HEAD` first; if empty, use `git rebase --skip` to drop the fully-superseded commit cleanly (ProjectHephaestus PR #1365 / issue #1357, 2026-06-15) |
...
| ProjectHephaestus | 2026-06-15, PR #1365 / issue #1357 (branch `1357-auto-impl`): PR #1358 and PR #1365 both implemented CIDriver SRP decomposition creating identical new files (`ci_driver_core.py`, `ci_driver_phases.py`, `ci_driver_helpers.py`, `ci_driver_overrides.py`, `impl_phase_runner.py` + 5 test files). PR #1358 merged first. When PR #1365 rebased onto main, all 10 files conflicted as `CONFLICT (add/add)`. Resolved by writing main's version to each conflicted file (`git show REBASE_HEAD:<path> > /tmp/X && cat /tmp/X > path`), `git add`, then verified `git diff --cached REBASE_HEAD` was empty → `git rebase --skip`. Branch tip became identical to `origin/main` (`git diff origin/main HEAD` showed zero output). PR closed as superseded. | verified-local |

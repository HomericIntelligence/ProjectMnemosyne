...
| **Verification** | verified-ci (function-local / version-guard / Windows-guard / lazy-exports; section G `__all__` alignment / eager re-export / subset-assertion pin for ProjectHephaestus issue #1511, merged via PR #1605); verified-local (surface-pin-stale fix, access-time deprecation-warning implementation — section F executed end-to-end with all tests passing locally); **unverified** planning-guidance (COMPATIBILITY.md version-stamping within section G derived from a plan for ProjectHephaestus issue #1513, not yet executed in CI; section H — adding the companion `__dir__()` to a PEP 562 lazy-loader package; derived from a plan for ProjectHephaestus issue #1512, not yet executed in CI). PR #1605 CI green is based on the completed PR context and local merge commit evidence, not live GitHub API polling in this learn environment. |
...
- **Promoting undeclared public-looking symbols to the package surface — audit-finding fix (verified-ci for __all__ alignment; unverified for COMPATIBILITY.md version-stamping)**: One or more existing, public-looking functions are present in a submodule but absent from the package `__init__.py`'s `__all__`, so `from pkg import sym` fails for external users while internal callers work around it by importing from the submodule directly. Key verified patterns from ProjectHephaestus issue #1511 / PR #1605 (`hephaestus/cli/__init__.py` aligned with `hephaestus/cli/utils.py`): **expand scope** from the named symbols to ALL missing symbols when the same defect class applies — fixing all four (`DRY_RUN_HELP_CAVEAT`, `add_dry_run_arg`, `add_github_throttle_args`, `configure_github_throttle_from_args`) prevented re-triggering the same audit finding; treat the submodule's existing `__all__` as the stable public-surface contract for the package barrel; choose **eager re-export** (`from hephaestus.cli.utils import (…)` in `__init__.py`) when the source module pulls only stdlib — no `_LAZY_EXPORTS` infrastructure needed; **no COMPATIBILITY.md change** is needed when the package has no per-symbol table (only a tier-level entry at the package level); pin the test with a **subset assertion** (`set(utils.__all__) - set(cli.__all__)`) plus package-attribute checks so parallel PRs adding new symbols cannot break it; always include `test_import_surface.py`/`test_automation_boundary.py` in the verification set. Additional planning traps (from a plan for ProjectHephaestus issue #1513 — still unverified): stamping the COMPATIBILITY.md "Added" version by guessing from the latest git tag; missing a non-local strict-equality surface-pin test; building on an unverified stability-tier premise; getting the re-export binding wrong for `monkeypatch`/`mock.patch` paths. See section G.
...
#### G. Promoting undeclared public-looking symbols to the package surface (audit-finding fix) — verified-ci for __all__ alignment; unverified for COMPATIBILITY.md version-stamping

> **Verification:** `verified-ci` for `__all__` alignment, eager re-export, subset-assertion pin, package-attribute checks, and import-surface/boundary checks — these steps shipped in ProjectHephaestus PR #1605 for issue #1511 (`hephaestus/cli/__init__.py` aligned with `hephaestus/cli/utils.py`). Local merge commit evidence: `38838663f2cbdb8f63ca2e283c1964516da472e9` on ProjectHephaestus `origin/main`, titled `feat(cli): export add_dry_run_arg and DRY_RUN_HELP_CAVEAT (#1605)`, changing only `hephaestus/cli/__init__.py` and `tests/unit/cli/test_utils.py`. CI green is based on the completed PR context and merge commit evidence, not live GitHub API polling in this learn environment. COMPATIBILITY.md version-stamping guidance remains `unverified` — derived from a plan for ProjectHephaestus issue #1513, not yet executed in CI.
...
def test_cli_all_covers_module_all() -> None:
    """Subset assertion: every symbol in utils.__all__ must appear on the package."""
    ...
    for symbol in utils.__all__:
        assert hasattr(cli, symbol), f"hephaestus.cli missing package attribute {symbol}"
...
- **Assert real package attributes, not just import syntax**: a direct `from hephaestus.cli import X` assertion proves only the named symbol. The durable barrel contract is computed from `utils.__all__`, then each symbol is checked as a package attribute so a future `utils.__all__` addition cannot silently skip `hephaestus.cli`.
...
### Package barrel `__all__` alignment — verified-ci pattern

ProjectHephaestus PR #1605 / issue #1511 showed that package-barrel fixes must use the
submodule's existing `__all__` as the stable surface, not just the symbols named in the issue.
The final merged diff changed only:

- `hephaestus/cli/__init__.py`
- `tests/unit/cli/test_utils.py`

The barrel re-exported all four missing `utils.__all__` symbols:
...
The durable regression tests are:

- `TestCliBarrelExports::test_cli_all_covers_module_all`: computes
  `missing = set(utils.__all__) - set(cli.__all__)` and asserts every
  `utils.__all__` symbol is also a package attribute on `hephaestus.cli`.
- `TestCliBarrelExports::test_dry_run_symbols_reexport_identity`: asserts
  `cli.add_dry_run_arg is utils.add_dry_run_arg` and
  `cli.DRY_RUN_HELP_CAVEAT is utils.DRY_RUN_HELP_CAVEAT`, preventing package-vs-submodule
  patch-path divergence.

Verification evidence: ProjectHephaestus `origin/main` contains merge commit
`38838663f2cbdb8f63ca2e283c1964516da472e9`, titled
`feat(cli): export add_dry_run_arg and DRY_RUN_HELP_CAVEAT (#1605)`. CI green is based on the
completed PR context and local merge commit evidence, not live GitHub API polling in this learn
environment.
...
| ProjectHephaestus | Issue #1511 / PR #1605 — `[S14 API Design]` align `hephaestus/cli/__init__.py __all__` with `cli/utils.py __all__` | **verified-ci** (section G). Merge commit `38838663f2cbdb8f63ca2e283c1964516da472e9` on ProjectHephaestus `origin/main`, titled `feat(cli): export add_dry_run_arg and DRY_RUN_HELP_CAVEAT (#1605)`, changed only `hephaestus/cli/__init__.py` and `tests/unit/cli/test_utils.py`. Added 4 missing symbols (`DRY_RUN_HELP_CAVEAT`, `add_dry_run_arg`, `add_github_throttle_args`, `configure_github_throttle_from_args`) to both the import block and `__all__` in `hephaestus/cli/__init__.py`. Eager re-export (`cli/utils.py` is stdlib-only). No COMPATIBILITY.md change (no per-symbol table for `hephaestus.cli`). Added `test_cli_all_covers_module_all` (subset + package-attribute assertion) + `test_dry_run_symbols_reexport_identity` (identity check) in `tests/unit/cli/test_utils.py`. CI green is based on completed PR context and local merge commit evidence, not live GitHub API polling in this learn environment. |

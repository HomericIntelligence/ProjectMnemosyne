...
- "Replace low-risk compatibility wrappers with an allowlisted `__getattr__` delegate"
- "`IssueImplementer` has too many `self.phase_runner._xxx` wrapper methods — which can be dynamic?"
- "Will `patch.object(instance, name, ...)` still work if a private method is supplied by `__getattr__`?"
- "Which phase-runner wrappers must remain explicit because they are workflow/review seams?"
...
### Phase 15: Planning an allowlisted `__getattr__` delegate for low-risk compatibility wrappers (NEW in v1.10.0, PLANNING-ONLY)

> **Warning:** This phase is a **proposed workflow** captured from PLANNING ProjectHephaestus
> issue #1391. It was **NOT validated end-to-end** — no code was written, no issue text was
> fetched during this capture, no AST audit was re-run, no tests were run, and no CI confirmed it.
> Treat every step below as a hypothesis until implementation re-validates it on current code.
> The rest of this skill is `verified-ci`; this phase alone is unverified.

The #1391 plan proposed reducing `IssueImplementer` boilerplate by replacing audited,
low-risk one-call compatibility wrappers of the form `self.phase_runner._xxx(...)` with an
allowlisted `IssueImplementer.__getattr__` delegate. Explicit wrappers stay in place for
workflow/review methods that carry behavior, encode policy, or are heavily patched in tests.
This is a **planning-risk** capture, not a completed implementation.

#### 15a. Treat AST and patch inventories as plan-time evidence, not proof

An AST audit can identify wrappers that *look* like pure one-call delegates, and `rg` can count
`patch.object(...)` references. Those are useful starting points, but they drift. Before deleting
any wrapper, re-run the audit against the exact implementation branch and inspect each candidate
body. Every name added to `_PHASE_RUNNER_DELEGATES` must be proven immediately before deletion to
be a pure pass-through to `self.phase_runner` with no logging, branching, state mutation, retry
policy, label/CI behavior, exception translation, or review-loop semantics.

Do not infer "low-risk" only from lower patch frequency. A rarely patched wrapper can still be a
behavior-bearing workflow seam.

#### 15b. Keep explicit wrappers for behavior-bearing and heavily patched seams

The dynamic delegate is only for audited compatibility shims. Keep explicit methods when the
method participates in implementation workflow, review/PR loops, terminal label policy, state
transitions, or test seams with many direct patches. Explicit wrappers document behavior and give
tests a stable, inspectable method body; deleting them for line-count reduction raises review risk.

Reviewer rule: every explicit wrapper removal needs a written reason. "It delegates today" is
not enough; the plan must show it is allowlisted, pure, and not a workflow/review seam.

#### 15c. Prove `patch.object(instance, name, ...)` compatibility, not just runtime delegation

The compatibility claim depends on Python attribute lookup behavior: an instance attribute set by
`patch.object(instance, name, replacement)` should shadow `__getattr__`, and cleanup should restore
the missing attribute so later access falls back to `__getattr__` again. Pin that contract with
focused tests:

```python
def test_phase_runner_delegate_patch_object_shadowing(impl):
    with patch.object(impl, "_delegated_private_method", return_value="patched") as mock_method:
        assert impl._delegated_private_method() == "patched"
        mock_method.assert_called_once_with()

    # After patch cleanup, lookup falls back to phase_runner again.
    assert impl._delegated_private_method() == impl.phase_runner._delegated_private_method()
```

Also test that delegated methods bind to the current `impl.phase_runner`, not a stale phase-runner
captured at construction time, and that unknown private names raise `AttributeError`.

#### 15d. Avoid brittle structural tests

Do not approve tests that assert exact `__dict__` absence, class-map internals, or implementation
layout details unrelated to the compatibility contract. The durable contract is observable:
allowlisted names delegate to `impl.phase_runner`, instance-level patching shadows the dynamic
delegate and restores correctly, explicit wrappers remain explicit, and unknown names fail with
`AttributeError`.

#### 15e. Required verification before approving the implementation

The plan-time capture did **not** run these commands. The implementation PR should run them on
current code before claiming the refactor is safe:

```bash
pixi run pytest tests/unit/automation/test_implementer*.py -v
...
If mypy or lint tooling rejects dynamic private delegates, keep the wrappers or narrow the
dynamic mechanism. Do not waive type/static-tooling failures as "just private compatibility" unless
the project has an explicit policy for that exception.
...
| (PLANNING #1391) Treat an AST audit of one-call delegates as deletion proof | Planned to replace low-risk-looking `IssueImplementer` wrappers with an allowlisted `__getattr__` delegate based on a pre-edit AST audit | The audit was plan-time evidence only; implementation-time code can differ, and a one-call body may still be behavior-bearing if it is a workflow/review seam | Re-run and inspect the audit immediately before deletion; every `_PHASE_RUNNER_DELEGATES` name must be proven to be a pure `self.phase_runner` pass-through on current code |
| (PLANNING #1391) Equate lower patch count with lower risk | Patch inventory suggested some wrappers were patched less often, making them look safer to delegate dynamically | Patch counts drift and frequency is not the same as semantic risk; a rarely patched method can still control review workflow, state transitions, labels, or CI behavior | Use patch inventory as a review input, not a decision rule. Keep explicit wrappers for behavior-bearing workflow/review methods and heavily patched seams |
| (PLANNING #1391) Assume `patch.object(instance, name, ...)` remains compatible with a dynamic private delegate | The plan relied on instance-level patches shadowing `__getattr__` for every delegated private method | That compatibility was assumed, not proven in the learn capture. If patch cleanup does not restore fallback lookup, existing tests become flaky or silently exercise the wrong target | Add focused tests proving patch shadowing, post-context restore to the phase-runner delegate, current `impl.phase_runner` binding, and `AttributeError` for unknown names |
| (PLANNING #1391) Test class internals instead of the compatibility contract | Considered structural assertions about exact class dictionaries or delegate-map internals | Such tests are brittle and can fail under harmless implementation reshaping while missing the actual compatibility risk | Assert observable behavior: allowlisted names delegate, explicit wrappers remain explicit, instance patching works, and unknown names raise `AttributeError` |
| (PLANNING #1391) Assume type checker/static tooling accepts dynamic private delegates | The plan treated `__getattr__` delegation as a runtime-only compatibility move | Mypy, Ruff, or IDE/static tooling may reject or obscure dynamic private methods, especially when tests or callers access them directly | Require mypy and Ruff verification before approval; if tooling rejects the dynamic approach, keep explicit wrappers or narrow the allowlist |
...
| ProjectHephaestus | #1391 | Phase 15 — planning an allowlisted `IssueImplementer.__getattr__` delegate for pure `self.phase_runner` compatibility wrappers while preserving explicit behavior-bearing/review/workflow wrappers | ⚠️ **unverified — planning only, NOT executed** (no code, no tests, no CI; issue not fetched during capture) |
...
`refactoring`, `dry-principle`, `helper-methods`, `radiance`, `test-fakes`, `validation-wrappers`, `layout-kernels`, `tdd`, `code-quality`, `python`, `pytest`, `private-modules`, `test-structure`, `git-signing`, `importlib-metadata`, `srp`, `extract-method`, `lru-cache`, `mock-patch`, `rglob`, `dead-code-removal`, `constants`, `frozenset`, `drift`, `intentional-variant`, `core-extras-split`, `planning`, `stale-issue-body`, `inline-vs-fixture`, `remote-branch-divergence`, `hmac`, `test-helper-dedup`, `stale-duplicate-count`, `diff-before-merge`, `parameterize-defaults`, `print-summary`, `existing-dedup-home`, `patched-test-seam`, `thin-wrapper`, `free-function-extraction`, `automation-library-boundary`, `issueimplementer`, `phase-runner`, `dynamic-delegate`, `getattr`, `compatibility-wrapper`, `ast-audit`

## Version History

- **v1.10.0** (2026-06-26, **planning-only for the new phase / unverified**): Added Phase 15 — planning an allowlisted `IssueImplementer.__getattr__` delegate for audited, low-risk `self.phase_runner` compatibility wrappers, captured from ProjectHephaestus issue #1391 planning. Lessons: (1) AST and patch inventories are plan-time evidence only and must be re-run immediately before deletion; (2) every `_PHASE_RUNNER_DELEGATES` name must be proven to be a pure `self.phase_runner` pass-through on current code; (3) keep explicit wrappers for behavior-bearing workflow/review methods and heavily patched seams; (4) prove `patch.object(instance, name, ...)` shadows `__getattr__`, restores fallback lookup after context exit, binds to the current `impl.phase_runner`, and preserves `AttributeError` for unknown names; (5) avoid brittle tests asserting exact `__dict__`/delegate-map internals; (6) require focused implementer tests plus Ruff and mypy before approval. NOT executed end-to-end (no code, no tests, no CI; GitHub issue #1391 was not fetched during capture). Prior v1.9.0 snapshot archived to history.

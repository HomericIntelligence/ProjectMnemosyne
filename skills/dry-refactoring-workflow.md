...
ProjectHephaestus #1420, a `test_dir_does_not_import_or_warn` test popped `retry_with_jitter` from `__dict__` to prove
...
### Phase 25: PLANNING the FINAL cleanup-wave issue of a strictly-serialized epic whose deliverables assume infrastructure predecessor issues have NOT yet built (NEW in v1.20.0, PLANNING-ONLY — ProjectHephaestus #1823)

> **Verification — `unverified` (PLANNING-ONLY):** This phase was captured from PLANNING ProjectHephaestus
> issue #1823 — the **FINAL (14th) issue of a strictly-serialized epic (#1809)**. It was **NOT executed**:
> no code was written, no tests were run, no CI ran. Every lesson below is a PLANNING discipline. The plan's
> concrete `file:line` anchors were verified against the CURRENT tree, but the deliverable itself is a
> **gated blueprint** whose execution is blocked until 13 predecessor issues merge.

The #1823 issue body opened with `Depends on #1822` and was the last of 14 serialized sub-issues under epic
**#1809**, whose body states "the loop must never work two of these concurrently." Its deliverable #1 read
"the `*_core` functions are **already consumed by `stages/pr_review.py`** as worker-job bodies." On disk at
planning time, **all 13 predecessors (#1810–#1822) were OPEN**, and **none** of the named infrastructure
(`stages/pr_review.py`, `run_pipeline`, `PipelineConfig`, `PipelineScope`, `ADR-0006`) existed. The
deliverable's core premise was a **forward-looking assumption, not current fact** — this is the Phase
12/13/17/21 "stale issue anchor" family **escalated to the EPIC level**: not just the "Evidence:" section
but the **CORE PREMISE of the deliverables** describes a FUTURE state the epic has not yet reached.

#### 25a. The dependency-gate check is the FIRST planning action, not the last

When an issue body opens with `Depends on #<prev>` and belongs to an epic that serializes its sub-issues,
**VERIFY the entire dependency chain state BEFORE planning any code** — do not scope the extraction first and
check dependencies as an afterthought:
...
find hephaestus -name 'pr_review.py' -path '*stages*'   # → empty
grep -rn 'def run_pipeline\|class PipelineConfig\|class PipelineScope' hephaestus/   # → empty
ls docs/adr/0006-*.md 2>/dev/null   # → absent
```

If the chain is unbuilt, the issue's deliverables are describing a state the epic will only reach LATER. Do
not plan as if that state exists.

#### 25b. The correct output for a dependency-blocked final issue is a GATED-but-EXECUTABLE plan

Not a refusal, and not a plan that pretends the deps exist. **Emit the full concrete blueprint** (real
`file:line` anchors verified against the CURRENT tree), but make **Implementation-Order step 1 a MANDATORY
re-ground gate** that STOPS if the deps have not landed at execution time, and state plainly in the
**Objective** which predecessors must merge first:

```markdown
## Implementation Order
1. **GATE (re-ground).** Confirm #1810–#1822 are MERGED and `stages/pr_review.py` / `run_pipeline` /
   `PipelineConfig` / `PipelineScope` exist on disk. If ANY is absent, STOP — the deps have not landed;
   do not proceed.
2. …blueprint steps against the current tree…
```

#### 25c. The "pure cores" an extraction targets may ALREADY be free functions — grep before assuming

Do not assume an extraction is needed. In #1823 the target functions
`run_pr_review_analysis`, `gather_impl_review_context`, `review_pr_inline`, `run_address_fix_session`,
`resolve_addressed_threads`, `_parse_addressed_block` were **already module-level free functions** (extracted
earlier for the in-loop implementer step). The **ONLY blocker** to un-omitting was that they **cohabit a file
with a live `main()`** (an orchestration entrypoint that shells out to live `claude`/`gh`, which is why the
file is omit-listed). So the extraction is: **move the already-pure functions into a new `*_core.py` sibling
so the file that keeps the live `main()` stays omitted, while the core module becomes coverage-eligible** —
mirroring the established **#1357 precedent** (`pr_discovery.py` / `ci_check_inspector.py` split out of
`ci_driver.py`). Grep first (`grep -nE '^def (run_pr_review_analysis|review_pr_inline|…)' <file>`); if they
are already free functions, the task is a MOVE, not an EXTRACTION.

#### 25d. Omit-list "never positive at any point in the epic" is a git-HISTORY claim a single snapshot cannot fully verify

A single-snapshot planner CANNOT prove the omit-list count "never went positive at any point in the epic" —
the intervening #1820/#1821/#1822 reductions have not happened yet. The plan can only assert the **ISOLATED
contribution** of this one issue (**−2, 17→15**) and must **defer the cross-epic never-positive proof to
`git log -p -- pyproject.toml`** at execution time. **Flag this as an explicit assumption for the reviewer**;
do not state the never-positive invariant as verified.

#### 25e. Guardrail-triple mechanics for a module LEAVING the omit list

Removing a module from coverage `omit` touches a fixed set of guards (reinforces the coverage-omit phases):

- **`pyproject.toml`** `[tool.coverage.run].omit` — remove the entry.
- **`test_omit_allowlist.py`** `expected_modules` — remove the entry **and update the count comment**.
- **The orchestration-smoke docstring count** — update the prose count.
- **`test_omit_justification.py` needs NO edit** — it only checks that the REMAINING omitted modules each
  have a backing unit-test suite, so a REMOVED entry auto-passes.
- **`test_orchestration_smoke.py` `OMITTED_MODULES` is DERIVED dynamically from `pyproject.toml`** — it
  self-adjusts; only its prose count comment needs updating.
- **A wrapper module that RETAINS a live `main()` legitimately STAYS omitted** — you un-omit the **new
  `*_core.py` sibling**, not the wrapper. The point of the split is to make the pure logic coverage-eligible
  WITHOUT un-omitting the shell-out orchestration.

#### 25f. Patch-seam preservation via a re-export shim (reinforces Phase 14/21)

Symbols patched by module path — e.g. `patch("hephaestus.automation.pr_reviewer.run_pr_review_analysis")` —
must stay importable at the **OLD path** after the move, or every old-path patch target breaks. The wrapper
that keeps `main()` does:

```python
from .pr_review_core import (  # noqa: F401  (re-export: preserve patch seams at the old module path)
...
and its OWN methods call the **re-exported name** (not a fresh import from `pr_review_core`) so a
`patch("...pr_reviewer.<sym>")` at any existing call site still bites. Grep `patch(.*"pr_reviewer\.` (and
`patch.object`) BEFORE the move to enumerate every seam that must survive.

#### 25g. Acceptance gate (planning-only — the plan was NOT executed)

There is no test/CI evidence — this is a blueprint. The plan's own gate is:

```markdown
- GATE: #1810–#1822 MERGED and `stages/pr_review.py`/`run_pipeline`/`PipelineConfig`/`PipelineScope` present.
- Coverage-eligible `pr_review_core.py` exists; `pr_reviewer.py` retains `main()` + the re-export shim.
- `omit` −1 (the core sibling is NOT re-added); allowlist −1 + count comment; smoke docstring count −1.
- Every `patch("...pr_reviewer.<sym>")` seam still resolves (re-export shim present).
- Isolated omit contribution asserted as −2 (17→15); never-positive-across-epic deferred to
  `git log -p -- pyproject.toml` at execution.
```

Verification: **unverified — planning-only**; NO code, NO tests, NO CI. Sibling to the planning-only Phases
10/13/14/15/24.

### Phase 26: PLANNING a pure-function MOVE to a new sibling module when the moved function's INTERNAL calls are themselves patched test seams — the transitive-patch-seam repoint rule (NEW in v1.21.0, PLANNING-ONLY — ProjectHephaestus #1823 R1)

> **Verification — `unverified` (PLANNING-ONLY):** This phase was captured from PLANNING ProjectHephaestus
> issue #1823 at **R1** — the R0 plan received a **NOGO (grade C)** for a **CORRECTNESS ERROR** that is
> exactly the trap below. It was **NOT executed**: no code, no tests, no CI. Every lesson is a PLANNING
> discipline that EXTENDS Phase 14 (patched test seams) and **CORRECTS** Phase 25f's re-export-shim advice
> for the transitive case.

Phase 25f (and Phases 14/20/21) say: keep the patched name importable at the OLD module path via a re-export
shim (`from .new_core import f, g  # noqa: F401`) so `patch("...old_mod.<name>")` still resolves. That is
**NECESSARY but INSUFFICIENT**, and the R0 plan for #1823 **MISAPPLIED** it — it claimed a re-export shim on
the OLD module "keeps tests working" for a seam that reaches INTO a moved function. **It does not.** When you
MOVE a pure function `f` (e.g. `review_pr_inline`) that internally calls `g()` (e.g. `run_pr_review_analysis`,
`gh_pr_resolve_thread`) from `old_mod.py` into a new `new_core.py` sibling, a `patch("...old_mod.g")` seam
that fires while `f` runs **silently stops patching**.

#### 26a. WHY the shim is insufficient: name resolution follows the DEFINING module

A `patch("pkg.MODULE.name")` overrides **only** the binding looked up by the function whose **body reads**
`name`. Python resolves a module-level function's global lookups in **the module the function is DEFINED in**,
not the module it is imported/called from. So after `f` moves to `new_core`:

- `f`'s internal `g(...)` now resolves `g` in **`new_core`'s** namespace.
- A test doing `patch("pkg.old_mod.g")` while triggering `f` patches a name `f` **no longer reads** — the
  **mock never fires**. Worse: a mock that "isn't called" frequently **passes vacuously** (no
  `assert_called`), or `f` reaches a **real subprocess** (`gh`/`claude`) the test meant to stub.
- A **re-export shim** on the old module (`from .new_core import f, g`) fixes `from pkg.old_mod import f`
  **importers** — it makes the NAMES importable at the old path. It does **NOT** redirect `f`'s internal
  lookup of `g`. Re-export ≠ patch-target redirection.

#### 26b. The repoint decision is PER-PATCH-SITE — driven by where the READING function lives AFTER the move

Decide each patch site by locating the function whose body actually reads the patched name, then asking
whether THAT function moves:

- Patched name read by a function that **STAYS in the wrapper** (e.g. a class method
  `PRReviewer._review_pr`, `AddressReviewer._run_fix_session`) → **KEEP** the old `pkg.old_mod.name` path
  (the wrapper still owns that lookup; the re-export shim of Phase 25f suffices here).
- Patched name read by a function that **MOVES to the core** (`run_pr_review_analysis`, `review_pr_inline`,
  `run_address_fix_session`, `resolve_addressed_threads`) → **REPOINT** to `pkg.new_core.name`.

#### 26c. The trap is TRANSITIVE / multi-hop — trace the FULL call chain, patch the INNERMOST reader's module

A test's entry point can be a class method that **stays**, which **delegates** to a module-level function that
**moves**, which reads the patched name. The patch must target the module owning the **innermost function
that actually reads the name** (the core), even though the test never names the moved function:

```text
reviewer._resolve_addressed_threads(...)   # class method — STAYS in the wrapper
   └─ resolve_addressed_threads(...)        # module-level fn — MOVES to pr_review_core
        └─ reads gh_pr_resolve_thread       # ← the patched name lives with the MOVED reader
# => patch("...pr_review_core.gh_pr_resolve_thread"), NOT "...pr_reviewer.gh_pr_resolve_thread"
```

Another hop from the same issue:

```text
_run_impl_review_step → review_pr_inline (MOVES) → run_pr_review_analysis / gh_pr_review_post
# test_implementer_loop.py's patch("...pr_reviewer.run_pr_review_analysis")
#   must REPOINT to "...pr_review_core.run_pr_review_analysis"
#   even though the test never names review_pr_inline
```

#### 26d. The planning deliverable: a per-patch-site repoint TABLE (this is what prevents the NOGO)

BEFORE scoping any pure-function move, enumerate every seam and classify it:

```bash
grep -rn 'patch("pkg.<old_mod>\.' tests/    # every module-path patch into the old module
grep -rn 'patch\.object'          tests/    # object-attr patches (usually STAY — bound to the instance)
```

Then build the table — **this table IS the deliverable**:

| test site | patched name | function whose body READS it | does that fn MOVE? | resulting patch path |
| --------- | ------------ | ---------------------------- | ------------------ | -------------------- |
| `test_implementer_loop.py:…` | `run_pr_review_analysis` | `review_pr_inline` (via `_run_impl_review_step`) | **MOVES** | `pkg.pr_review_core.run_pr_review_analysis` |
| `test_pr_reviewer.py:…` | `gh_pr_review_post` | `PRReviewer._review_pr` (method) | **STAYS** | `pkg.pr_reviewer.gh_pr_review_post` (keep) |
| `test_address_review.py:…` | `gh_pr_resolve_thread` | `resolve_addressed_threads` | **MOVES** | `pkg.pr_review_core.gh_pr_resolve_thread` |

Also **CO-LOCATE cross-core references**: a moved `f` that calls a moved `g` should live in the **same** new
core module, so their mutual call resolves **inside** the core and `patch("pkg.new_core.g")` correctly
overrides it. Splitting `f` and `g` across two new modules re-creates the transitive trap at a new seam.

#### 26e. Acceptance gate (planning-only — the plan was NOT executed)

- **Assert the MOCKS FIRE.** Run the repointed test classes and require `mock.assert_called*` — do NOT accept
  "tests pass," because a broken patch commonly passes **vacuously** (the mock is never touched, nothing
  asserts it was, and the real path may be exercised instead).
- **Self-falsifying grep must return EMPTY:** no `patch("pkg.old_mod.<moved-inner-name>")` remains anywhere
  in `tests/` for any name whose reader moved to the core.

```bash
# must be EMPTY after repointing (for each moved inner name):
grep -rn 'patch("pkg.old_mod\.\(run_pr_review_analysis\|review_pr_inline\|resolve_addressed_threads\|gh_pr_resolve_thread\)' tests/
```

Verification: **unverified — planning-only** (ProjectHephaestus #1823 R1; R0 NOGO/grade-C for exactly this
error); NO code, NO tests, NO CI. Sibling to the planning-only Phases 10/13/14/15/24/25 and the load-bearing
correction to Phase 25f.
...
| (PLANNING #1823) Plan the extraction assuming `stages/pr_review.py` already consumes the `*_core` functions | The issue's deliverable #1 read "the `*_core` functions are already consumed by `stages/pr_review.py` as worker-job bodies," so the plan almost scoped the move to fit a consumer that would import them | On disk ALL 13 predecessors (#1810–#1822) were OPEN and NONE of the named infra (`stages/pr_review.py`, `run_pipeline`, `PipelineConfig`, `PipelineScope`, `ADR-0006`) existed — the deliverable's core premise was a FORWARD-LOOKING assumption, not current fact (the stale-issue-body lesson escalated to the whole epic's premise) | The dependency-gate check is the FIRST planning action, not the last: `gh issue view <n> --json state` for EVERY predecessor + `find`/`grep` the tree for every piece of infra the deliverables NAME, BEFORE scoping any code. When the chain is unbuilt, the deliverables describe a FUTURE state — do not plan as if it exists |
| (PLANNING #1823) Either refuse the dependency-blocked issue, or write a plan that pretends the deps exist | Two tempting extremes: (a) "cannot plan — #1822 is open, come back later"; (b) scope the extraction as though `stages/pr_review.py` and `run_pipeline` were present | (a) wastes the verifiable current-tree analysis (the `*_core` functions and the omit split ARE plannable now); (b) ships a blueprint whose step 1 imports a module that does not exist — it will fail at execution with no warning | Emit a GATED-but-EXECUTABLE plan: full concrete blueprint against the CURRENT tree, but Implementation-Order step 1 is a MANDATORY re-ground GATE that STOPS if the deps have not landed, and the Objective names which predecessors must merge first |
| (PLANNING #1823) Assume the "pure cores" still need EXTRACTING from their methods | The task read like a classic method→free-function extraction, so the instinct was to plan cutting method bodies out into free functions | `run_pr_review_analysis`/`gather_impl_review_context`/`review_pr_inline`/`run_address_fix_session`/`resolve_addressed_threads`/`_parse_addressed_block` were ALREADY module-level free functions (extracted earlier for the in-loop implementer step). The ONLY blocker to un-omitting was that they cohabit a file with a live `main()` | Grep (`grep -nE '^def <name>' <file>`) before assuming an extraction. If the cores are already free functions, the task is a MOVE to a new `*_core.py` sibling (so the `main()` file stays omitted while the core becomes coverage-eligible) — mirroring the #1357 `pr_discovery.py`/`ci_check_inspector.py`-out-of-`ci_driver.py` precedent, NOT a fresh extraction |
| (PLANNING #1823) State the omit-list "never went positive at any point in the epic" invariant as verified | The epic's success criterion is that omit-count monotonically shrinks; the plan was tempted to assert this issue upholds "never positive across the epic" | A SINGLE snapshot cannot prove a git-HISTORY claim — the intervening #1820/#1821/#1822 reductions have not happened, so the cross-epic trajectory is unknowable at plan time | Assert only the ISOLATED contribution (−2, 17→15) and DEFER the cross-epic never-positive proof to `git log -p -- pyproject.toml` at execution time; flag it as an explicit assumption for the reviewer, do not claim it as verified |
| (PLANNING #1823) Un-omit the wrapper module (the one keeping `main()`) and edit `test_omit_justification.py` | Instinct was to remove the WHOLE original module from `omit` and touch every omit-related guard including the justification test | The wrapper RETAINS a live `main()` that shells out to `claude`/`gh` — it legitimately STAYS omitted; and `test_omit_justification.py` only checks REMAINING omitted modules have backers, so a removed entry auto-passes with no edit | Un-omit the NEW `*_core.py` sibling, not the wrapper. The guardrail-triple to edit is `pyproject.toml` omit (−entry), `test_omit_allowlist.py` `expected_modules` (−entry + count comment), and the orchestration-smoke docstring count; `test_omit_justification.py` needs NO edit, and `test_orchestration_smoke.py` `OMITTED_MODULES` is DERIVED from pyproject so only its prose count comment changes |
| (PLANNING #1823) Move the functions and let the wrapper import them fresh from `pr_review_core` | The wrapper's methods would just `from .pr_review_core import run_pr_review_analysis` and call it | Symbols patched by module path (`patch("hephaestus.automation.pr_reviewer.run_pr_review_analysis")`) would no longer resolve at the OLD path after the move — every old-path patch seam breaks | Preserve the seam with a re-export shim: `from .pr_review_core import (...)  # noqa: F401` in the wrapper, and have the wrapper's methods call the RE-EXPORTED name so `patch("...pr_reviewer.<sym>")` still bites (reinforces Phase 14/21). Grep `patch(.*"pr_reviewer\.` + `patch.object` BEFORE the move to enumerate every seam that must survive |
| (PLANNING #1823 R1 — the R0 NOGO error) Rely on a re-export shim on the OLD module to keep a `patch("...old_mod.g")` seam working after moving the function `f` that calls `g()` | The R0 plan asserted "the re-export shim on `pr_reviewer` keeps `patch("...pr_reviewer.run_pr_review_analysis")` working" even though the calling function `review_pr_inline` MOVED to `pr_review_core` | A re-export shim redirects IMPORTS (`from pkg.old_mod import g`), NOT the moved `f`'s internal global lookup of `g`. After the move, `f` resolves `g` in `pr_review_core`'s namespace, so `patch("...pr_reviewer.g")` patches a name `f` no longer reads — the mock SILENTLY never fires (and often passes vacuously or hits a real `gh`/`claude` subprocess). This was the CORRECTNESS ERROR that earned the R0 plan a NOGO / grade C | REPOINT the patch to the module that DEFINES the function reading the name (`patch("...pr_review_core.g")`), PER-SITE — trace the FULL call chain to find the innermost reader. A re-export shim is only sufficient when the reader is a method that STAYS in the wrapper |
| (PLANNING #1823 R1) Classify a transitive patch seam by the TEST's entry point rather than the innermost reader | The test called `reviewer._resolve_addressed_threads(...)` (a class method that STAYS), so the seam "looked like" it belonged to `pr_reviewer` | The method DELEGATES to the module-level `resolve_addressed_threads`, which MOVES to `pr_review_core` and is what actually reads `gh_pr_resolve_thread` — the patch must target the core, not the wrapper the test names | Build a per-patch-site table [test site | patched name | function whose body READS it | does it MOVE? | resulting path] and trace the full call chain; also CO-LOCATE a moved `f` and the moved `g` it calls in the SAME new core module so `patch("...new_core.g")` resolves inside the core. Acceptance gate: assert the MOCKS FIRE (not just tests pass), and a `patch("...old_mod.<moved-inner-name>")` grep over `tests/` must return EMPTY |
...
| ProjectHephaestus | #1823 (epic #1809) | Phase 25 — PLANNING the FINAL (14th) issue of a strictly-serialized epic whose deliverable #1 ("`*_core` functions already consumed by `stages/pr_review.py`") ASSUMED infra that predecessors had NOT built (all 13 of #1810–#1822 OPEN; `stages/pr_review.py`/`run_pipeline`/`PipelineConfig`/`PipelineScope`/`ADR-0006` absent on disk). Dependency-gate check FIRST (`gh issue view --json state` per predecessor + `find`/`grep` the tree for every named infra symbol); emit a GATED-but-EXECUTABLE plan (full blueprint against the CURRENT tree, Implementation-Order step 1 a MANDATORY re-ground GATE that STOPS if deps absent); the "pure cores" were ALREADY free functions (`run_pr_review_analysis`/`gather_impl_review_context`/`review_pr_inline`/`run_address_fix_session`/`resolve_addressed_threads`/`_parse_addressed_block`) blocked only by cohabiting a live-`main()` file → MOVE to a `pr_review_core.py` sibling (#1357 precedent) so the `main()` wrapper stays omitted while the core becomes coverage-eligible; omit "never positive across the epic" deferred to `git log -p -- pyproject.toml` (assert only the isolated −2, 17→15); guardrail-triple (`pyproject.toml` omit / `test_omit_allowlist.py` `expected_modules`+count / smoke docstring count; `test_omit_justification.py` no edit; `OMITTED_MODULES` derived); patch-seam re-export shim (`from .pr_review_core import (...)  # noqa: F401`) so `patch("...pr_reviewer.<sym>")` still bites | ⚠️ **unverified — planning only, NOT executed** (no code, no tests, no CI; the plan is a gated blueprint blocked until #1810–#1822 merge) |
| ProjectHephaestus | #1823 (R1) | Phase 26 — the transitive-patch-seam REPOINT rule (load-bearing CORRECTION to Phase 25f). R1 plan after the R0 plan got a NOGO/grade-C for the CORRECTNESS ERROR of claiming a re-export shim on the OLD module keeps a `patch("...pr_reviewer.g")` seam working after MOVING the calling function `f` (`review_pr_inline`, `resolve_addressed_threads`) into `pr_review_core`. A `patch("MODULE.name")` overrides only the binding read by the function DEFINED in `MODULE`; a moved `f` resolves its internal `g()` in the CORE's namespace, so the old-path patch silently never fires (passes vacuously / hits a real `gh`/`claude` subprocess). Repoint PER-PATCH-SITE by tracing the FULL (transitive) call chain to the innermost reader: KEEP `...pr_reviewer.name` when a method that STAYS reads it (`PRReviewer._review_pr`), REPOINT to `...pr_review_core.name` when a moved function reads it; deliverable = a [test site | patched name | reader fn | moves? | patch path] table; co-locate moved `f`+`g` in one core; acceptance = mocks FIRE (`assert_called*`) + a `patch("...pr_reviewer.<moved-inner-name>")` grep over `tests/` returns EMPTY | ⚠️ **unverified — planning only, NOT executed** (no code, no tests, no CI; R1 plan, R0 NOGO grade C for this exact transitive patch-seam error) |
...
`refactoring`, `dry-principle`, `helper-methods`, `radiance`, `test-fakes`, `validation-wrappers`, `layout-kernels`, `tdd`, `code-quality`, `python`, `pytest`, `private-modules`, `test-structure`, `git-signing`, `importlib-metadata`, `srp`, `extract-method`, `lru-cache`, `mock-patch`, `rglob`, `dead-code-removal`, `constants`, `frozenset`, `drift`, `intentional-variant`, `core-extras-split`, `planning`, `stale-issue-body`, `inline-vs-fixture`, `remote-branch-divergence`, `hmac`, `test-helper-dedup`, `stale-duplicate-count`, `diff-before-merge`, `parameterize-defaults`, `print-summary`, `existing-dedup-home`, `patched-test-seam`, `thin-wrapper`, `free-function-extraction`, `automation-library-boundary`, `script-to-library-shim`, `shim-vs-delete`, `output-contract-preservation`, `byte-for-byte-stdout`, `granular-vs-main`, `private-import-boundary`, `tdd-green-first-inversion`, `duplicate-bearing-script`, `deprecated-api-removal`, `breaking-change`, `lazy-loader`, `lazy-imports`, `deprecation-guard-test`, `pep-562`, `all-surfaces`, `absence-guard-test`, `repo-wide-grep-gauntlet`, `compatibility-doc`, `migration-doc`, `stale-plan-anchor`, `wrong-filename-anchor`, `indirection-root`, `grep-current-state`, `verified-against-disk-not-exempt`, `log-format-constants`, `configure-cli-logging`, `title-body-mismatch`, `stale-issue-title`, `task-plan-review-pipeline`, `authoritative-source-of-truth`, `approved-plan`, `cli-log-format`, `boundary-placement`, `intentional-variant-carveout`, `grep-gauntlet`, `env-coercion`, `read-timeout-env`, `duplicate-reader-delegate`, `thin-delegate`, `unused-import`, `f401`, `i001-import-resort`, `import-time-read`, `post-edit-tree`, `ruff-check-fix`, `stale-test-path-anchor`, `relocated-guard-tests`, `no-tests-ran-false-pass`, `find-before-run`, `canonical-public-helper`, `context-manager`, `contextmanager`, `try-finally`, `acquire-release`, `resource-pattern`, `status-tracker`, `slot`, `yield-none`, `cm-cannot-return`, `none-guard-inside-with`, `guard-release-on-none`, `behavior-bearing-finally`, `throttle-side-effect`, `initial-msg-per-site`, `strict-superset-wrapper`, `opt-in-timeout`, `leak-grep-gate`, `self-falsifying-grep`, `plan-anchor-off-by-one`, `scripted-reindent`, `sentinel-marker`, `ast-parse-verify`, `e501-fstring-hand-wrap`, `double-release`, `failure-path-helper`, `tiny-module-merge`, `module-consolidation`, `merge-into-sibling`, `title-body-set-mismatch`, `approved-plan-set-authoritative`, `pure-relocation`, `orphan-reference-grep`, `repoint-not-rewrite`, `d102-on-target`, `lint-rule-incidentally-exempt`, `protocol-move`, `runtime-checkable`, `method-docstring`, `plan-self-contradiction`, `modify-vs-delete`, `verification-commands-disambiguate`, `pixi-no-cov-addopts`, `partial-selection-coverage-gate`, `mypy-duplicate-module`, `bare-pixi-mypy`, `relocated-guard-test-path`, `source-deleted-check`, `omit-allowlist`, `single-consumer-merge`, `stale-reference-fix-step`, `no-op-plan-step`, `orphan-grep`, `same-prefix-symbol-exclusion`, `planner-claude`, `stale-line-count-and-consumer`, `protocol-host-docstring-reference`, `add-only-missing-imports`, `drop-dead-import`, `path-list-guard-delete-vs-rename`, `read-text-guard`, `not-omit-listed-check`, `concurrent-futures`, `drain-loop`, `extract-scaffold-not-body`, `generator-helper`, `yield-from-done`, `wait-first-completed`, `silent-busy-loop`, `exponential-backoff`, `mapping-read-only-param`, `caller-driven-termination`, `futures-pop`, `wait-grep-false-positive`, `ruff-f401-decides-import`, `import-time-stays`, `single-edit-reindent`, `lambda-type-inference`, `magicmock-spec-future`, `named-def-over-lambda`, `module-level-patch-target`, `split-test-file-nonexistent-path`, `audit-snapshot-already-resolved`, `post-snapshot-merged-pr`, `verification-only-guard`, `anti-drift-guard-test`, `terminal-stale-anchor`, `already-consolidated-upstream`, `print-worker-summary`, `free-function-not-base-class`, `advisory-not-binding-placement`, `placement-divergence-correct`, `green-first-guard`, `tdd-inversion`, `no-source-change`, `grep-gate`, `read-text-grep-gate`, `per-class-delegation-assert`, `object-new-unbound-method`, `skip-init-seam`, `patch-at-consumer-module-path`, `assert-called-once-with`, `byte-for-byte-args`, `intentional-variant-different-signature`, `implementation-summary-printer`, `planner-print-summary-variant`, `d205-module-docstring`, `one-line-summary-then-blank`, `split-test-filenames`, `test-pr-reviewer-split`, `ls-grep-discover-test-filenames`, `pytest-exit-4-silent-false-negative`, `no-cov-addopts`, `serialized-epic`, `dependency-gate-first`, `depends-on-chain`, `final-cleanup-wave-issue`, `forward-looking-deliverable`, `core-premise-stale`, `epic-level-stale-anchor`, `gated-but-executable-plan`, `re-ground-gate`, `mandatory-gate-step-1`, `blueprint-not-refusal`, `predecessors-must-merge-first`, `already-free-functions`, `move-not-extract`, `pure-core-sibling`, `core-py-split`, `live-main-stays-omitted`, `coverage-eligible-sibling`, `1357-precedent`, `omit-never-positive-git-history`, `single-snapshot-cannot-prove-history`, `isolated-omit-contribution`, `defer-to-git-log`, `guardrail-triple`, `test-omit-allowlist-count-comment`, `test-omit-justification-no-edit`, `omitted-modules-derived`, `orchestration-smoke-docstring-count`, `wrapper-stays-omitted`, `re-export-shim`, `noqa-f401-re-export`, `patch-seam-old-path`, `patch-module-path-must-resolve`, `stages-pr-review`, `run-pipeline`, `pipeline-config`, `pipeline-scope`, `adr-0006`, `epic-1809`, `issue-1823`, `transitive-patch-seam`, `patch-seam-repoint`, `re-export-shim-insufficient`, `name-resolution-defining-module`, `moved-function-internal-lookup`, `per-patch-site-repoint`, `innermost-reader-module`, `multi-hop-delegation`, `full-call-chain-trace`, `patch-target-redirection`, `import-vs-patch-target`, `mock-silently-never-fires`, `vacuous-pass`, `assert-mocks-fire`, `patch-old-path-empty-grep`, `co-locate-cross-core-refs`, `pure-core-sibling-move`, `patch-module-path-vs-object`, `r0-nogo-correctness-error`, `phase-25f-correction`, `pr-review-core`, `review-pr-inline`, `resolve-addressed-threads`, `repoint-table-deliverable`

## Version History

- **v1.21.0** (2026-07-04, **planning-only for the new phase / unverified**): Added Phase 26 — PLANNING a pure-function MOVE to a new sibling module when the moved function's INTERNAL calls are themselves patched test seams — the **transitive-patch-seam repoint rule**, the load-bearing CORRECTION to Phase 25f (re-export shim). Captured from PLANNING ProjectHephaestus issue #1823 at **R1**, after the R0 plan received a **NOGO (grade C)** for the CORRECTNESS ERROR the phase now guards against: R0 asserted a re-export shim on the OLD module (`pr_reviewer`) "keeps `patch("...pr_reviewer.run_pr_review_analysis")` working" even though the calling function `review_pr_inline` MOVED into `pr_review_core`. NOT executed (no code, no tests, no CI). Five sub-sections: (26a) WHY the shim is insufficient — a `patch("pkg.MODULE.name")` overrides ONLY the binding read by the function whose body reads `name`, and Python resolves a module-level function's globals in THE MODULE THE FUNCTION IS DEFINED IN; so a moved `f` resolves its internal `g()` in the NEW core's namespace and `patch("...old_mod.g")` patches a name `f` no longer reads → the mock SILENTLY never fires (often passing vacuously, or reaching a real `gh`/`claude` subprocess); a re-export shim fixes IMPORTERS (`from pkg.old_mod import f`), not patch-target redirection; (26b) the repoint decision is PER-PATCH-SITE — KEEP the old `pkg.old_mod.name` path when the reader STAYS in the wrapper (a class method like `PRReviewer._review_pr` / `AddressReviewer._run_fix_session`), REPOINT to `pkg.new_core.name` when the reader MOVES (`run_pr_review_analysis`, `review_pr_inline`, `run_address_fix_session`, `resolve_addressed_threads`); (26c) the trap is TRANSITIVE / multi-hop — a test's entry point can be a method that STAYS (`reviewer._resolve_addressed_threads`) delegating to a module-level function that MOVES (`resolve_addressed_threads`) that reads the patched name (`gh_pr_resolve_thread`); patch the module of the INNERMOST reader (the core), even though the test never names the moved function (`test_implementer_loop.py`'s `patch("...pr_reviewer.run_pr_review_analysis")` must repoint to `...pr_review_core` because `_run_impl_review_step → review_pr_inline(MOVES) → run_pr_review_analysis`); (26d) the deliverable is a per-patch-site TABLE [test site | patched name | function whose body READS it | does it MOVE? | resulting patch path], built from `grep -rn 'patch("pkg.<old_mod>.' tests/`, plus CO-LOCATING cross-core references (a moved `f` calling a moved `g` in the SAME new core module so `patch("...new_core.g")` resolves inside the core); (26e) acceptance gate — assert the MOCKS FIRE (`mock.assert_called*`), not just that tests pass (a broken patch passes vacuously), and a self-falsifying grep `patch("pkg.old_mod.<moved-inner-name>")` over `tests/` must return EMPTY. Added Phase 26 (5 sub-sections 26a–26e), 3 Failed Attempts rows, a Verified On entry, a description item (26), tags, and this Version History entry. Bumped frontmatter `version` to `1.21.0`; `date` stays `2026-07-04`; frontmatter `verification` stays `unverified` (per the v1.10.0+ convention), with Phase 26 carrying its own inline planning-only label. Sibling to the planning-only Phases 10/13/14/15/24/25; explicit CORRECTION to Phase 25f. Built on the branch of the still-open PR #2970 (v1.20.0 / Phase 25), preserving Phase 25 in full. Prior v1.20.0 snapshot archived to history.
- **v1.20.0** (2026-07-04, **planning-only for the new phase / unverified**): Added Phase 25 — PLANNING the FINAL (14th) issue of a strictly-serialized epic whose deliverables ASSUME infrastructure predecessor issues have NOT yet built, captured from PLANNING ProjectHephaestus issue #1823 (the last sub-issue of epic #1809, whose body forbids working two sub-issues concurrently). NOT executed end-to-end (no code, no tests, no CI). Seven sub-sections: (25a) the dependency-gate check is the FIRST planning action, not the last — when an issue opens with `Depends on #<prev>` in a serialized epic, VERIFY the whole chain BEFORE scoping code (`gh issue view <n> --json state` per predecessor + `find`/`grep` the tree for every infra symbol the deliverables NAME); in #1823 all 13 predecessors (#1810–#1822) were OPEN and NONE of `stages/pr_review.py`/`run_pipeline`/`PipelineConfig`/`PipelineScope`/`ADR-0006` existed, so the deliverable's core premise ("already consumed by `stages/pr_review.py`") was FORWARD-LOOKING — the Phase 12/13/17/21 stale-anchor family escalated to the EPIC-level CORE PREMISE; (25b) the correct output is a GATED-but-EXECUTABLE plan (full blueprint against the CURRENT tree, Implementation-Order step 1 a MANDATORY re-ground GATE that STOPS if deps absent, Objective names which predecessors must merge first) — NOT a refusal and NOT a plan that pretends the deps exist; (25c) the "pure cores" (`run_pr_review_analysis`/`gather_impl_review_context`/`review_pr_inline`/`run_address_fix_session`/`resolve_addressed_threads`/`_parse_addressed_block`) were ALREADY free functions — grep before assuming; the ONLY blocker to un-omitting was cohabiting a live-`main()` file, so it is a MOVE to a `pr_review_core.py` sibling (#1357 precedent) so the `main()` wrapper stays omitted while the core becomes coverage-eligible; (25d) omit-list "never positive at any point in the epic" is a git-HISTORY claim a single snapshot cannot verify (the intervening #1820/#1821/#1822 reductions have not happened) — assert only the ISOLATED −2 (17→15) and defer never-positive to `git log -p -- pyproject.toml` at execution, flagged as an assumption; (25e) guardrail-triple for a module LEAVING the omit list — `pyproject.toml` omit (−entry), `test_omit_allowlist.py` `expected_modules` (−entry + count comment), orchestration-smoke docstring count; `test_omit_justification.py` needs NO edit (only checks REMAINING omitted modules have backers), `test_orchestration_smoke.py` `OMITTED_MODULES` is DERIVED from pyproject so it self-adjusts, and a wrapper retaining `main()` legitimately STAYS omitted (un-omit the `*_core.py` sibling, not the wrapper); (25f) patch-seam preservation via re-export shim (reinforces Phase 14/21) — symbols patched by module path (`patch("...pr_reviewer.run_pr_review_analysis")`) must stay importable at the OLD path, so the wrapper does `from .pr_review_core import (...)  # noqa: F401` and its methods call the re-exported name; (25g) acceptance gate is the plan's own re-ground GATE — there is no test/CI evidence. Added Phase 25 (7 sub-sections 25a–25g), 7 Failed Attempts rows, a Verified On entry, a description item (25), tags, and this Version History entry. Bumped frontmatter `version` to `1.20.0` and `date` to `2026-07-04`; frontmatter `verification` stays `unverified` (per the v1.10.0+ convention), with Phase 25 carrying its own inline planning-only label. Sibling to the planning-only Phases 10/13/14/15/24. Prior v1.19.0 snapshot archived to history.

...
description: 'Add numerical gradient validation tests for batch_norm2d_backward in
  inference mode (training=False), covering grad_gamma and grad_beta via finite differences.
  Use when: gradient checks only cover training=True, inference code path uses running_mean/running_var,
  or issue asks to extend coverage to inference mode.'
category: testing
date: 2026-03-15
version: 1.0.0

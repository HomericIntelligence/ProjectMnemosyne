...
description: "Fix pattern for elif\u2192if fallthrough when a primary JSON file exists\
  \ with a null key, silently preventing fallback to a secondary file. Use when: T5\
  \ subtests fail with 'all required tiers failed' despite tiers completing, or any\
  \ if/elif chain where primary file exists but has empty/null data."
category: debugging
date: 2026-03-13
version: 1.0.0

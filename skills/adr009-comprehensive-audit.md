...
description: 'Audit all Mojo test files in a family/module for ADR-009 compliance
  when fixing one violation, then create a dedicated CI group. Use when: (1) fixing
  an ADR-009 violation and sibling files may also exceed the limit, (2) no dedicated
  CI group exists for the test family, (3) split files are buried in an overloaded
  CI group.'
category: ci-cd
date: 2026-03-07
version: 1.0.0

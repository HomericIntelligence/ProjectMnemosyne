...
description: 'Verify and close a review-fix task when a docs-only PR has no actual
  code changes needed and CI failures are pre-existing. Use when: a review-fix plan
  concludes no changes are required, CI failures are unrelated to the PR diff, or
  a docs-only PR needs final pre-commit verification before merge.'

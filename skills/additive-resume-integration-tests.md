...
date: '2026-03-19'
version: 1.1.0
absorbed: [tier-state-additive-resume-tests]
---
...
## TierStateMachine Layer Tests

The `tier_states` checkpoint field is populated **only** by `TierStateMachine`. Running
`SubtestStateMachine` alone leaves `tier_states` empty; a tier is implicitly `"pending"` if absent
from `tier_states`, regardless of whether subtests/runs completed.
...
### Helper: `_simulate_tier_to_complete`

Combines `SubtestStateMachine` + `TierStateMachine` to drive a tier fully to COMPLETE:
...
### Helper: `make_noop_tier_actions` (full TierState enum coverage)
...
### Assert tier_states additive across resume
...
### Structural guarantee: only explicitly run tiers in tier_states
...
### TestAdditiveResumeTierStates test scenarios

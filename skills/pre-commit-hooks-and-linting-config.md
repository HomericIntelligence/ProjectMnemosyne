...
| **Outcome** | Consolidated from 53 narrow skills; TOML/Ruff parity verified-ci; Bandit `pass_filenames: false` pattern added (verified-local); pip-audit ledger checker patterns added (verified-precommit); failed-hook commit-abort / phantom-signature trap added (verified-precommit); `SKIP=<hook>`-in-one-job-but-enforced-fail-fast-in-another false-green trap added (verified-ci) |
...
- A repo documents / runs a hook with `SKIP=<hook>` in ONE job (treating it as advisory), yet a SEPARATE required CI job enforces that same hook in a distinct fail-fast step WITHOUT the skip — e.g. the `precommit-benchmark` job and the documented local command both run `SKIP=mojo-format pixi run pre-commit run --all-files` (mojo-format is advisory because the Mojo 1.0.0b2 / 0.26.1 formatter has known bugs), BUT the required `pre-commit` job has a "Run mojo format (fail-fast)" step that runs mojo-format with NO skip. Your local `SKIP=mojo-format` run passes → a FALSE GREEN while the required `pre-commit` check fails with `Failed - hook id: mojo-format - files were modified by this hook`. Before pushing, reproduce CI's EXACT command: run the FULL suite with NO skip (`pixi run pre-commit run --all-files` → rc=0), and after applying `pixi run mojo format <files>` re-run the full no-skip suite so the NEXT fail-fast file does not surface as a second red
...
#### A hook skipped in ONE job is enforced fail-fast in ANOTHER — `SKIP=<hook>` locally is a false green

A repo can mark a hook "advisory" in one place while a DIFFERENT required job enforces it.
When the advisory form is what the docs and one CI job run, reproducing that exact form
locally gives a FALSE GREEN — because the required job runs the hook WITHOUT the skip in a
separate fail-fast step.

**Concrete case (ProjectOdyssey PR #5584, the package rename):**

- The `precommit-benchmark` CI job AND the documented local invocation both run
  `SKIP=mojo-format pixi run pre-commit run --all-files` — mojo-format is treated as advisory
  because the Mojo 1.0.0b2 / 0.26.1 formatter has known bugs.
- BUT the SEPARATE, REQUIRED `pre-commit` CI job has a distinct step **"Run mojo format
  (fail-fast)"** that runs mojo-format WITH NO skip. On the rename PR it failed:
  `Mojo Format ... Failed - hook id: mojo-format - files were modified by this hook`
  (it reformatted `tests/odyssey/integration/test_packaging.mojo` and
  `tests/odyssey/autograd/test_dropout_backward.mojo`).
- The local check had used the `SKIP=mojo-format` form and passed → a false green; the
  required `pre-commit` check was red.

**Fix — reproduce CI's EXACT command (no skip) before pushing:**

```bash
# 1. Apply the formatting the fail-fast step wants:
pixi run mojo format tests/odyssey/integration/test_packaging.mojo \
                     tests/odyssey/autograd/test_dropout_backward.mojo
#    (or: pixi run pre-commit run mojo-format --all-files)

# 2. Commit the reformats.
git add -A && git commit -m "style(mojo): apply mojo-format fail-fast reflow"

# 3. CRUCIAL: re-run the FULL suite with NO skip — this is what the required
#    `pre-commit` job runs; SKIP=mojo-format would hide the next fail-fast file.
pixi run pre-commit run --all-files          # expect rc=0

# 4. Confirm the CI check went green:
gh pr checks 5584                            # `pre-commit  pass`
```

Re-running the full no-skip suite (step 3) matters: mojo-format fail-fast stops at the first
offending file, so fixing only the two files CI named without re-running the whole suite risks
whack-a-mole — the next file surfaces as a fresh red on the next push.

**General rule:** when a repo marks a hook "advisory" or skips it via `SKIP=<hook>` in one
job, CHECK whether another REQUIRED job enforces the same hook in a separate (often fail-fast)
step. Reproduce each required job's EXACT command locally — for a `pre-commit` required check
that means the FULL `pre-commit run --all-files` with NO `SKIP=`. On PR #5584 this was the 3rd
distinct CI failure on the same rename PR (1: `precommit-benchmark` → ruff-format 1 file;
2: the directory-move issue; 3: `pre-commit` → mojo-format fail-fast); getting green required
reproducing each job's EXACT command locally, not just the documented advisory form. This is
DISTINCT from the sub-agent `SKIP=hook-id` bypass above (there the orchestrator forgot to
re-run a hook a sub-agent skipped on an older-GLIBC host); here the SKIP is baked into the
repo's own documented command yet a second required job enforces it anyway.

Verified by ProjectOdyssey PR #5584 — after `pixi run mojo format <files>` + full no-skip
`pre-commit run --all-files` (rc=0), the required `pre-commit` check went green
(`gh pr checks 5584` showed `pre-commit  pass`).
...
| Trust a local `SKIP=mojo-format pre-commit run --all-files` pass as the pre-commit gate | Ran the repo's DOCUMENTED command `SKIP=mojo-format pixi run pre-commit run --all-files` (also what the `precommit-benchmark` job runs, since the Mojo formatter has known bugs); it passed → assumed the required `pre-commit` check would pass too | The SEPARATE required `pre-commit` CI job has a distinct "Run mojo format (fail-fast)" step that runs mojo-format WITHOUT the skip; it failed `Failed - hook id: mojo-format - files were modified by this hook` on `tests/odyssey/integration/test_packaging.mojo` + `tests/odyssey/autograd/test_dropout_backward.mojo` — the local SKIP form was a FALSE GREEN | A hook marked advisory/skipped in ONE job may be enforced fail-fast in ANOTHER required job. Reproduce CI's EXACT command: run the FULL suite with NO skip (`pixi run pre-commit run --all-files` → rc=0). After `pixi run mojo format <files>`, re-run the whole no-skip suite (fail-fast stops at the first file — otherwise the next file surfaces as a fresh red) |
...
| ProjectOdyssey | PR #5584 (package rename): local `SKIP=mojo-format pre-commit run --all-files` (the repo's documented + `precommit-benchmark`-job command) passed, but the SEPARATE required `pre-commit` job's "Run mojo format (fail-fast)" step ran mojo-format with no skip and failed on `test_packaging.mojo` + `test_dropout_backward.mojo`; fixed via `pixi run mojo format <files>` + full no-skip `pre-commit run --all-files` (rc=0) → `gh pr checks 5584` showed `pre-commit  pass` | verified-ci |

...
|-------|-------|
| **Date** | 2026-06-27 |
| **Objective** | A hardcoded `HomericIntelligence/ProjectMnemosyne` slug was scattered across skills (/advise, /learn) AND the Python automation pipeline, so only that org could use the shared knowledge base and every /learn PR targeted the canonical upstream. Make it work for any GitHub user. |
...
UPSTREAM_SLUG = f"{UPSTREAM_OWNER}/ProjectMnemosyne"
...
resolve_mnemosyne_target() {
  ...
  if [ -z "$owner" ] || [ "$owner" = "$upstream_owner" ]; then
    echo "$upstream_owner/ProjectMnemosyne"; return
  fi
  if gh repo view "$owner/ProjectMnemosyne" --json name >/dev/null 2>&1; then
    echo "$owner/ProjectMnemosyne"; return
  fi
  gh repo fork "$upstream_owner/ProjectMnemosyne" --clone=false >/dev/null 2>&1 || true
...
5. **Broaden evidence/confirmation regexes.** Change `HomericIntelligence/ProjectMnemosyne` literals to `[A-Za-z0-9._-]+/ProjectMnemosyne` so a push to a fork still counts as confirmation in /learn evidence checks.
...
| Keep the hardcoded test assertion | Test asserted `"HomericIntelligence/ProjectMnemosyne" in clone_cmd` | Once resolution is dynamic the clone command contains the resolved fork slug, so the assertion breaks | Patch `resolve_mnemosyne_target` in the test and assert the resolved slug instead of the upstream literal |
...
| Narrow confirmation regex | Left evidence regex matching only `HomericIntelligence/ProjectMnemosyne` | A push to a user's fork (`<login>/ProjectMnemosyne`) no longer matched, so /learn never registered confirmation | Broaden to `[A-Za-z0-9._-]+/ProjectMnemosyne` |
...
|---------|---------|---------|
| ProjectHephaestus | PR #1668 (org-aware ProjectMnemosyne resolution), all required CI green | resolver `hephaestus/github/mnemosyne_repo.py`; tests `tests/unit/github/test_mnemosyne_repo.py` |

...
description: 'Move config files from a dedicated config/ directory into tests/ when
  config is only consumed by test infrastructure. Use when: config dir contains files
  superseded by a test-side system, loaders have hardcoded config/ paths, or a legacy
  config directory needs full elimination.'

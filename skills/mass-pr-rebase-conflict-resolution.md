...
description: 'Rebase multiple DIRTY/CONFLICTING PRs against main, resolve merge conflicts,
  and fix pre-commit format CI failures without local mojo format. Use when: (1) many
  PRs show DIRTY merge state after main advances, (2) mojo format fails in CI but
  can''t run locally due to GLIBC incompatibility, (3) same files conflict across
  many PRs.'
category: ci-cd
date: 2026-03-07
version: 1.0.0

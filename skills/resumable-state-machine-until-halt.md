...
description: 'Fix --until semantics in resumable state machines: sentinel exception
  that transitions state before stopping, additive CLI tier merging, ephemeral CLI
  arg preservation. Use when advance() leaves subtest/tier stuck at prior state because
  sentinel exception fires before the state update.'

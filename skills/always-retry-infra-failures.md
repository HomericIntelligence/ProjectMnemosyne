...
description: 'Fix retry logic bugs where a flag-gated retry conflates infra crashes
  with bad-grade results, discarding valid judge results. Use when: removing a retry
  flag to make behavior unconditional, fixing state machine reset logic that incorrectly
  resets completed runs, or debugging checkpoint semantics with multiple terminal
  state meanings.'
category: debugging
date: 2026-03-14
version: 1.0.0

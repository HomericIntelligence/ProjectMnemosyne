...
| Model-cap classifier | `detect_model_usage_cap(*texts)` in `claude_invoke.py` | Three independent signals: a regex matching "reached your" + a non-session/usage model-name + "limit", `/usage-credits`, or `switch models with /model`; the lookahead keeps session/usage limits owned by the wait-path detectors |

...
description: 'Fix pre-commit hooks that use pass_filenames: false when they should
  use pass_filenames: true, removing hardcoded directory args so pre-commit passes
  only changed files. Use when: hooks always scan entire directories instead of only
  changed files, or linter entry commands have hardcoded directory paths.'
category: ci-cd
date: 2026-03-05
version: 1.0.0

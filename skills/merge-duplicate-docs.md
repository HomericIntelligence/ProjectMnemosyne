...
description: 'Merge two overlapping markdown docs into one canonical source, preserving
  unique content and fixing all cross-references. Use when: two files cover the same
  topic (DRY violation), one has visual/summary content and the other has detailed
  specs, or you need a single authoritative reference.'

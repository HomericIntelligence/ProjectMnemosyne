...
description: 'Audit completed ADR-009 test file splits for dropped tests, missing
  headers, and secondary violations. Use when: a split was previously done but tests
  are still failing or the issue remains open, the post-split test count doesn''t
  match the original file''s count, or other test files in the same CI group also
  violate ADR-009.'
category: ci-cd
date: 2026-03-07
version: 1.0.0

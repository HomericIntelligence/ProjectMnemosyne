...
> **Warning:** This sub-pattern (added in v1.5.0, from ProjectHephaestus issue #1506 R2 — a **critical D / NOGO** on the R1 re-plan) is **unverified** — it is a PLAN, the code was NOT executed or CI-validated. Treat as a hypothesis until CI confirms. The v1.0.0 coredump material remains `verified-local`; the v1.1.0–v1.5.0 sub-patterns are plan-only.

This is the **second** NOGO on the SAME #1506 session and a distinct, more severe lesson than the v1.4.0 MINOR tightenings. Where R1's NOGO was driven by incidental test coverage, the R2 NOGO was a **critical defect in the guard's iteration scope**: the guard's own acceptance test would FAIL on the shipped tree.

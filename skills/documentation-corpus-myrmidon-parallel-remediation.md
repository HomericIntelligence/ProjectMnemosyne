...
# Gate: ALL FOUR passes must return zero matches.
# If pass 1 is clean but pass 2, pass 3, or pass 4 surfaces hits, that is the
# expected first-iteration result — fix and re-run all four until all return zero.
```

**Why four passes, not one giant union**: pass 1 and pass 2 are literal text scrubs; pass 3 and pass 4 are case-insensitive because editorial and review-process phrases vary in capitalization (`UNDERSTATED`, `Understated`, `understated`; `Per Review`, `per review`). Folding them into a single grep loses the case distinction or explodes the alternation list. Keeping them as four separate invocations also makes the failure mode legible — a fail on pass 2 tells you instantly that the bibliography-suffix class is the problem, not the semantic or authorial-review class; a fail on pass 4 isolates review-process vocabulary (`per review`, `after review`) from direct correction words.
